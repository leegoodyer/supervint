...
// Any path that treats a missing sv:sub:<clientId> as "brand new" needs this
// first — a clientId can be missing its live record because it was soft-
// deleted (admin/delete) rather than merged elsewhere, in which case its old
// data — plan, email, customerId, everything — is still sitting recoverable
// in sv:deleted:<clientId>. Building a fresh {} record on top of that instead
// of reviving it is exactly how a real Stripe customerId got silently wiped:
// admin/grant granted a plan directly to a clientId that had been deleted
// minutes earlier, found nothing at sv:sub:, and had no idea there was
// anything to recover.
//
// Deliberately does NOT revive a record that has mergedInto set — that
// clientId's data already moved forward to a different, live clientId, and
// reviving the stale pre-merge copy here would create a duplicate/conflict
// rather than recovering anything.
export async function getOrRevive(kv, clientId) {
  const existing = await kv.get(`sv:sub:${clientId}`);
  if (existing) return { record: existing, revived: false };

  const deletedRecord = await kv.get(`sv:deleted:${clientId}`);
  if (deletedRecord && !deletedRecord.mergedInto) {
    const { deletedAt, ...revived } = deletedRecord;
    return { record: revived, revived: true };
  }

  return { record: {}, revived: false };
}
